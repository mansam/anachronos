...
from anachronos.player import Player
import anachronos.logic
from anachronos.map import MapGenerator, Map
from anachronos.game import GameState
import anachronos.game
import sys
...
def move_unit(state, player):
	...
	if unit and unit.player.name is player.name:
		...
		if path:
			...
			player.paradox += anachronos.logic.calculate_distance(unit.location, tile)/2
			player.moves.append(anachronos.game.MoveAction(player.move_number, player, unit, unit.location, tile, path))
		if not path:
...
def attack(state, player):
	...
	if unit and unit.player.name is player.name:
		...
		if unit.can_attack(tile):
			...
			player.paradox += anachronos.logic.calculate_distance(unit.location, tile)/2
			player.attacks.append(anachronos.game.AttackAction(player.attack_number, player, unit, unit.location, tile))
		else:
